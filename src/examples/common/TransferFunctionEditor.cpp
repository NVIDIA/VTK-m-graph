...
void TransferFunctionEditor::setValueRange(const math::float2 &vr)
{
  m_valueRange = m_defaultValueRange = vr;
  m_tfnChanged = true;
}

math::float2 TransferFunctionEditor::getValueRange()
{
  return m_valueRange;
}

std::vector<math::float4> TransferFunctionEditor::getSampledColorsAndOpacities(
    int numSamples)
{
  std::vector<math::float4> sampledColorsAndOpacities;

  const float dx = 1.f / (numSamples - 1);

  for (int i = 0; i < numSamples; i++) {
    sampledColorsAndOpacities.push_back(
        math::float4(interpolateColor(*m_tfnColorPoints, i * dx),
...
math::float3 TransferFunctionEditor::interpolateColor(
    const std::vector<ColorPoint> &controlPoints, float x)
{
  auto first = controlPoints.front();
  if (x <= first.x)
    return math::float3(first.y, first.z, first.w);
  ...
  for (uint32_t i = 1; i < controlPoints.size(); i++) {
    ...
    if (x <= current.x) {
      ...
      return (1.0f - t) * math::float3(previous.y, previous.z, previous.w)
          + t * math::float3(current.y, current.z, current.w);
    }
  }

  auto last = controlPoints.back();
  return math::float3(last.x, last.y, last.z);
}
...
void TransferFunctionEditor::updateTfnPaletteTexture()
{
  ...
  std::vector<math::float4> palette =
      getSampledColorsAndOpacities(textureWidth);
...
void TransferFunctionEditor::drawEditor()
{
    ...
    for (int i = 0; i < m_tfnColorPoints->size(); ++i) {
      ...
      // drag color control point
        ...
        if (i > 0 && i < m_tfnColorPoints->size() - 1) {
          ...
          (*m_tfnColorPoints)[i].x = math::clamp((*m_tfnColorPoints)[i].x,
    ...
    for (int i = 0; i < m_tfnOpacityPoints->size(); ++i) {
      ...
      if (ImGui::IsMouseDoubleClicked(1) && ImGui::IsItemHovered()) {
        ...
        (*m_tfnOpacityPoints)[i].y =
            math::clamp((*m_tfnOpacityPoints)[i].y, 0.0f, 1.0f);
        if (i > 0 && i < m_tfnOpacityPoints->size() - 1) {
          (*m_tfnOpacityPoints)[i].x += delta.x / width;
          (*m_tfnOpacityPoints)[i].x = math::clamp((*m_tfnOpacityPoints)[i].x,
  ...
  if (ImGui::IsMouseDoubleClicked(0) && ImGui::IsItemHovered()) {
    const float x = math::clamp(
        (mouse_x - canvas_x - margin - scroll_x) / (float)width, 0.f, 1.f);
    const float y = math::clamp(
